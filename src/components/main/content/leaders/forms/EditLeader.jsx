...
const EditLeader = ({ onEdit }) => {
    const leader = useSelector((state) => state.leaders.selected);

    const [name, setName] = useState(leader.name);
    const [icon, setIcon] = useState(leader.icon);
    const [intro, setIntro] = useState(leader.intro);

    const [abilityName, setAbilityName] = useState(leader.ability?.name || "");
    const [ability, setAbility] = useState("");
    const [abilities, setAbilities] = useState(leader.ability?.attributes || []);

    const [agendaName, setAgendaName] = useState(leader.agenda?.name || "");
    const [agenda, setAgenda] = useState("");
    const [agendas, setAgendas] = useState(leader.agenda?.attributes || []);

    const [attribute, setAttribute] = useState("");
    const [attributes, setAttributes] = useState(leader.attributes || []);

    const [historic, setHistoric] = useState("");
    const [historics, setHistorics] = useState(leader.civilizations?.historic || []);

    const [geographic, setGeographic] = useState("");
    const [geographics, setGeographics] = useState(leader.civilizations?.geographic || []);

    const [strategic, setStrategic] = useState("");
    const [strategics, setStrategics] = useState(leader.civilizations?.strategic || []);

    const [startBias, setStartBias] = useState("");
    const [startBiases, setStartBiases] = useState(leader.start_bias || []);

    const [momento, setMomento] = useState("");
    const [momentos, setMomentos] = useState(leader.momentos || []);
